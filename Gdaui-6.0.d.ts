...
import type * as Gda from './Gda-6.0.js';
...
export interface DataProxy extends Gtk.Widget {
    ...
    // Has conflict: performAction(action: Action): void
    // Has conflict: setWriteMode(mode: DataProxyWriteMode): boolean
    // Has conflict: supportsAction(action: Action): boolean
...
export module DataCellRendererInfo {
     ...
     */
    export interface StatusChangedSignalCallback {
        (path: string, requestedAction: Gda.ValueAttribute): void
    }
...
export interface DataCellRendererInfo {
    ...
    on(sigName: "status-changed", callback: DataCellRendererInfo.StatusChangedSignalCallback): number
    once(sigName: "status-changed", callback: DataCellRendererInfo.StatusChangedSignalCallback): number
    off(handlerId: number): number
    emit(sigName: "status-changed", requestedAction: Gda.ValueAttribute, ...args: any[]): void
...
export interface DataProxyInfo extends Atk.ImplementorIface, Gtk.Buildable, Gtk.Orientable, Gtk.ToolShell {
     ...
     * @param action a #GdauiAction action
     */
    getItem(action: Action): Gtk.ToolItem
...
export interface EntryShell extends Atk.ImplementorIface, Gtk.Buildable, Gtk.Orientable {
     ...
     */
    packEntry(entry: Gtk.Widget): void
    /**
     * Defines the color to be used when `de` displays an invalid value. Any value not
     * between 0. and 1. will result in the default hard coded values to be used (grayish).
     * @param red the red component of a color
     ...
     * @param alpha the alpha component of a color
     */
    setInvalidColor(red: number, green: number, blue: number, alpha: number): void
...
export interface DataProxyInterface {
    ...
    supportsAction: (iface: DataProxy, action: Action) => boolean
    performAction: (iface: DataProxy, action: Action) => void
